...
bool doProcessThreadPC(struct inlineHookItem *item, struct pt_regs *regs, int action)
{
	int offset;
	int i;
...
void processThreadPC(pid_t tid, struct inlineHookItem *item, int action)
{
	struct pt_regs regs;
...
pid_t freeze(struct inlineHookItem *item, int action)
{
	int count;
	pid_t tids[1024];
	pid_t pid;
...
bool isExecutableAddr(uint32_t addr)
{
	FILE *fp;
	char line[1024];
	uint32_t start;
...
struct inlineHookItem *findInlineHookItem(uint32_t target_addr)
{
	int i;
...
void deleteInlineHookItem(int pos)
{
...
void doInlineUnHook(struct inlineHookItem *item, int pos)
{
...
void doInlineHook(struct inlineHookItem *item)
{
	mprotect((void *) PAGE_START(CLEAR_BIT0(item->target_addr)), PAGE_SIZE * 2, PROT_READ | PROT_WRITE | PROT_EXEC);
